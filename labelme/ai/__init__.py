...
ROOT = FILE.parents[2]  # labelme root directory
# print('========FILE ==========', FILE)
# print('========ROOT ==========', ROOT)
...
class SegmentAnythingModelVitB(SegmentAnythingModel):
    ...
    def __init__(self):
        super().__init__(
             encoder_path= ROOT / 'weights/sam_speed.quantized.encoder.onnx',
             decoder_path= ROOT / 'weights/sam_speed.quantized.decoder.onnx',
...
class SegmentAnythingModelVitL(SegmentAnythingModel):
    ...
    def __init__(self):
        super().__init__(
            encoder_path= ROOT / 'weights/sam_balanced.quantized.encoder.onnx',
            decoder_path= ROOT / 'weights/sam_balanced.quantized.decoder.onnx',
            # encoder_path=gdown.cached_download(
...
class SegmentAnythingModelVitH(SegmentAnythingModel):
    ...
    def __init__(self):
        super().__init__(
            encoder_path= ROOT / 'weights/sam_accuracy.quantized.encoder.onnx',
            decoder_path= ROOT / 'weights/sam_accuracy.quantized.decoder.onnx',
            # encoder_path=gdown.cached_download(
...
class EfficientSamVitT(EfficientSam):
    ...
    def __init__(self):
        super().__init__(
            encoder_path= ROOT / 'weights/efficient_sam_speed_encoder.onnx',
            decoder_path= ROOT / 'weights/efficient_sam_speed_decoder.onnx',
            # encoder_path=gdown.cached_download(
...
class EfficientSamVitS(EfficientSam):
    ...
    def __init__(self):
        super().__init__(
            encoder_path= ROOT / 'weights/efficient_sam_accuracy_encoder.onnx',
            decoder_path= ROOT / 'weights/efficient_sam_accuracy_decoder.onnx',
            # encoder_path=gdown.cached_download(
